...
#if (BOARD_SELECT == BOARD_PAC5523AEVK1_1_1SHUNT_PWR2)
void init_diff_amp_gain(void)
{
    ...
    pac5xxx_tile_register_write(ADDR_CFGAIO6, ((AIO6789_MODE_GAIN << 6) | (AIO6_GAIN << 3) | AIO6789_MUX_AB6));                          // Gain Mode
...
void config_seq_foc(void)
{
    ...
    pac5xxx_timer_b_ccctr0_value_set(motor_ptr->control.motor_period_ticks >> 1);                              // Set value for AS0 trigger
    pac5xxx_timer_b_ccctr1_value_set(motor_ptr->control.motor_period_ticks >> 1);                              // Set value for AS1 trigger
    ...
    motor_ptr->oneshunt.taccr0_value = (motor_ptr->control.motor_period_ticks >> 1);
    motor_ptr->oneshunt.taccr1_value = (motor_ptr->control.motor_period_ticks >> 1);
    pac5xxx_timer_a_ccctr0_value_set(motor_ptr->control.motor_period_ticks >> 1);                              // Set value for AS0 trigger
    pac5xxx_timer_a_ccctr1_value_set(motor_ptr->control.motor_period_ticks >> 1);                              // Set value for AS1 trigger
#endif

    // Dummy conversions are added to each sequence to work around a bug in the DTSE
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AIO32, 0, 0);    // Convert M1_I, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(1, ADC0, EMUX_AIO32, 0, SEQ_END);    // Dummy Conversion, then change CAFE MUX to M1-I
    ...
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB6,   0,           0);    // Dummy Conversion, read Iavg, then change CAFE MUX to M1-Iavg
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB6,   0,           0);    // Dummy Conversion, read Iavg, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(9, ADC0, EMUX_AIO32, 0,     SEQ_END);    // Dummy Conversion, then change CAFE MUX to M1-I
    ...
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AIO32, 0,           0);    // Dummy Conversion, read vbus, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB6,   0,           0);    // Dummy Conversion, read Iavg, then change CAFE MUX to M1-Iavg
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB6,   0,           0);    // Dummy Conversion, read Iavg, then change CAFE MUX to M1-Iavg
    pac5xxx_dtse_seq_config(9, ADC0, EMUX_AIO32, 0,     SEQ_END);    // Dummy Conversion, then change CAFE MUX to M1-I
...
//#error: POWER_MEASURE_METHOD_2 not supported BOARD_EP5223LVM1_1_1SHUNT
    ...
    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AIO32, 0,           0);    // Convert M1-I, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AIO32, ADC_IRQ0_EN, 0);    // Dummy Conversion, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(5, ADC6, EMUX_AIO32, 0,           0);    // Dummy Conversion, read vbus, then change CAFE MUX to M1_I
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AIO32, 0,           0);    // Read vbus, then change CAFE MUX to M1-Iavg
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB6,   0,           0);    // Dummy Conversion, read Iavg, then change CAFE MUX to M1-Iavg
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB6,   0,           0);    // Dummy Conversion, read Iavg, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(9, ADC0, EMUX_AIO32, 0,     SEQ_END);    // Dummy Conversion, then change CAFE MUX to M1-I
#endif
...
void config_seq_sim(void)
{
    ...
    PAC55XX_ADC->DTSETRIGENT8TO11.FORCE8 = 0;          // Reset to 0
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG8CFGIDX = 0;     // DTSE Trigger 8 Sequence Configuration Entry Index
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG8EDGE = 1;       // Trigger rising edge

    pac5xxx_timer_b_ccctr0_value_set(motor_ptr->control.motor_period_ticks - 4);            // Set value for AS0 trigger
#else
    PAC55XX_ADC->DTSETRIGENT0TO3.FORCE0 = 0;          // Reset to 0
    PAC55XX_ADC->DTSETRIGENT0TO3.TRIG0CFGIDX = 0;     // DTSE Trigger 0 Sequence Configuration Entry Index
    ...
    // Dummy conversions are added to each sequence to work around a bug in the DTSE
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AB7, 0,           0);       // Convert Vout, then change CAFE MUX to M1-Vu
    pac5xxx_dtse_seq_config(1, ADC0, EMUX_AB8, 0,           0);       // Dummy Conversion, then change CAFE MUX to M1-Vu
    pac5xxx_dtse_seq_config(2, ADC0, EMUX_AB8, 0,           0);       // Convert M1-Vu, then change CAFE MUX to M1-Vv
    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AB9, 0,           0);       // Dummy Conversion, then change CAFE MUX to M1-Vv
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AB9, 0,           0);       // Convert M1-Vv, then change CAFE MUX to M1-Vw
    pac5xxx_dtse_seq_config(5, ADC6, EMUX_AB9, 0,           0);       // Dummy Conversion, then change CAFE MUX to M1-Vw
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AB7, ADC_IRQ0_EN, 0);       // Convert M1-Vw, then change CAFE MUX to M1-Vu, generate ADC interrupt
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB7, 0,     SEQ_END);       // Dummy conversion, then change CAFE MUX to M1-Vu
}
...
#endif // #if (BOARD_SELECT == BOARD_PAC5523AEVK1_1_1SHUNT)
