...
#if (BOARD_SELECT == BOARD_PAC5523AEVK1_1_1SHUNT)
...
void config_seq_foc(void)
{
    ...
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AIO32, 0, SEQ_END);                             // Dummy conversion, then change CAFE MUX to M1-Iu
#endif
    ...
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG8CFGIDX = 0;       // DTSE Trigger 0 Sequence Configuration Entry Index
    ...
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AIO32, 0, 0);    // Convert M1_Iu, then change CAFE MUX to M1-I
    ...
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG9CFGIDX = 3;       // DTSE Trigger 1 Sequence Configuration Entry Index
    ...
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AIO32, 0, 0);    // Dummy Conversion, then change CAFE MUX to M1-I
...
// Configure ADC input pin PG7(AD0) to read analog input from AFE and PF6(AD6) to read VBUS
    ...
    pac5xxx_dtse_seq_config(1, ADC0, EMUX_AIO32, ADC_IRQ0_EN,           SEQ_END);    // Dummy Conversion, then change CAFE MUX to M1-Iu
...
#if (MODE_1SHUNT_SELECT == MODE_1SHUNT_ASYMMETRIC_TIMER_55XX_ONLY)
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG8EDGE = 0;      // Disabled
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG9EDGE = 0;      // Disabled
#else
    PAC55XX_ADC->DTSETRIGENT0TO3.TRIG1EDGE = 0;         // Disabled    
#endif

#if (MODE_1SHUNT_SELECT == MODE_1SHUNT_ASYMMETRIC_TIMER_55XX_ONLY)
    PAC55XX_ADC->DTSETRIGENT8TO11.FORCE8 = 0;            // Reset to 0
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG8CFGIDX = 0;       // DTSE Trigger 0 Sequence Configuration Entry Index
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG8EDGE = 1;         // Trigger falling edge
...
#if (MODE_1SHUNT_SELECT == MODE_1SHUNT_ASYMMETRIC_TIMER_55XX_ONLY) || (MODE_1SHUNT_SELECT == MODE_1SHUNT_ASYMMETRIC)
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AB7, 0, 0);                 // Convert M1-Vw, then change CAFE MUX to M1-Vu, no ADC interrupt
#else
