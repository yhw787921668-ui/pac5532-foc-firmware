...
#if (BOARD_SELECT == BOARD_PAC5532EVK1)
...
void config_seq_foc(void)
{
    ...
    pac5xxx_adc_config_io(0x10 | 0x40 | 0x01 | 0x20);    // 1、0x10，配置PF4为GPIO模式，Vdc采样，在GPIO配置为ADC4
                                                         // 2、0x40，配置PF6为GPIO模式，在GPIO配置为ADC6
                                                         // 3、0x01，配置PG7为GPIO模式，在GPIO配置为ADC0
	                                                       // 4、0x20，配置PF5为GPIO模式，Torq采样，在GPIO配置为ADC5
    ...
    pac5xxx_dtse_seq_config(0, ADC4, EMUX_AIO10 | DIFFAMPS_HOLD, 0,           0);     // Convert Vout, then change CAFE MUX to M1-Iu
    ...
    pac5xxx_dtse_seq_config(6, ADC0, EMUX_AIO10 | DIFFAMPS_HOLD, 0,           0);     // Convert M1-Iw, then change CAFE MUX to M1-Iu, generate ADC interrupt
    pac5xxx_dtse_seq_config(7, ADC6, EMUX_AIO10 | DIFFAMPS_HOLD, 0,           0);     // Dummy conversion, then change CAFE MUX to M1-Iu
    
    pac5xxx_dtse_seq_config(8, ADC4, EMUX_AIO10,                 0,         0);  
    pac5xxx_dtse_seq_config(9, ADC5, EMUX_AIO10,                 0,         0);  
    pac5xxx_dtse_seq_config(10, ADC5, EMUX_AIO10,      ADC_IRQ0_EN,         0);  
    pac5xxx_dtse_seq_config(11, ADC6, EMUX_AIO10,                 0,     SEQ_END);  		
		
//		// Configure ADC input pin PG7(AD0) to read analog input from AFE and PF6(AD6) to read VBUS
//		pac5xxx_adc_config_io(0x10 | 0x40 | 0x01 | 0x20);    // 1、0x10，配置PF4为GPIO模式，在GPIO配置为ADC4
//                                                         // 2、0x40，配置PF6为GPIO模式，在GPIO配置为ADC6
//                                                         // 3、0x01，配置PG7为GPIO模式，在GPIO配置为ADC0
//	                                                       // 4、0x20，配置PF5为GPIO模式，在GPIO配置为ADC5
//	
//    PAC55XX_ADC->DTSETRIGENT0TO3.FORCE0 = 0;         // DTSE模式下，自动转化，非强制转化
//    PAC55XX_ADC->DTSETRIGENT0TO3.TRIG0CFGIDX = 0;    // 指引序列号 0
//    PAC55XX_ADC->DTSETRIGENT0TO3.TRIG0EDGE = 2;      // PWMA0，下降沿触发
//    
//    // Dummy conversions are added to each sequence to work around a bug in the DTSE
//    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AIO10 | DIFFAMPS_HOLD, 0,           0);     // Convert Vout, then change CAFE MUX to M1-Iu
//    pac5xxx_dtse_seq_config(1, ADC0, EMUX_AIO10 | DIFFAMPS_HOLD, 0,           0);     // Dummy Conversion, then change CAFE MUX to M1-Iu
//    pac5xxx_dtse_seq_config(2, ADC0, EMUX_AIO32 | DIFFAMPS_HOLD, 0,           0);     // Convert M1-Iu, then change CAFE MUX to M1-Iv
//    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AIO32 | DIFFAMPS_HOLD, 0,           0);     // Dummy Conversion, then change CAFE MUX to M1-Iv
//    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0,           0);     // Convert M1-Iv, then change CAFE MUX to M1-Iv
//    pac5xxx_dtse_seq_config(5, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0,           0);     // Dummy Conversion, then change CAFE MUX to M1-Iw
//    pac5xxx_dtse_seq_config(6, ADC5, EMUX_AB5                  , ADC_IRQ0_EN, 0);     // PF5,Torque Signal
//    pac5xxx_dtse_seq_config(7, ADC5, EMUX_AB5                  , 0,           0);     // Dummy Conversion                                                                                // Dummy conversion, then change CAFE MUX to M1-Iu
//    pac5xxx_dtse_seq_config(8, ADC4, EMUX_AB4                  , 0,           0);     // PF4,Vbus
//    pac5xxx_dtse_seq_config(9, ADC4, EMUX_AB4                  , 0,     SEQ_END);     // Dummy Conversion 
...
#endif // #if (BOARD_SELECT == BOARD_PAC5532EVK1)
