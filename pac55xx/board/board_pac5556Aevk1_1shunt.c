...
#if (BOARD_SELECT == BOARD_PAC5556AEVK1_1SHUNT)
...
void config_seq_foc(void)
{
    ...
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AB11, 0,           0);     // Dummy Conversion, read vbus, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB11, 0,           0);     // Dummy Conversion, read vms, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB11, 0,           0);     // Dummy Conversion, read vms, then change CAFE MUX to vms
    ...
    PAC55XX_ADC->DTSETRIGENT8TO11.FORCE9 = 0;            // Reset to 0
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG9CFGIDX = 3;       // DTSE Trigger 1 Sequence Configuration Entry Index
    PAC55XX_ADC->DTSETRIGENT8TO11.TRIG9EDGE = 1;         // Trigger rising edge

    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AIO32, 0,           0);    // Convert M1-I, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AIO32, ADC_IRQ0_EN, 0);    // Dummy Conversion, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(5, ADC6, EMUX_AIO32, 0,           0);    // Dummy Conversion, read vbus, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AB11, 0,           0);    // Convert vbs, read vbus, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB11, 0,           0);     // Dummy Conversion, read vms, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB11, 0,           0);     // Convert vms, read vms, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(9, ADC0, EMUX_AIO32, 0,     SEQ_END);    // Dummy Conversion, then change CAFE MUX to M1-I
...
void config_seq_ipd(uint32_t sequencer_trigger_value)
{
    ...
    pac5xxx_timer_b_ccctr0_value_set(sequencer_trigger_value);                              // Set value for AS0 trigger
...
void config_seq_sim(void)
{
    ...
    pac5xxx_dtse_seq_config(6, ADC0, EMUX_AB11, ADC_IRQ0_EN, 0);       // Convert M1-Vw, then change CAFE MUX to M1-Vu, generate ADC interrupt
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB11, 0,           0);    // Dummy Conversion, read vbus, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB11, 0,           0);     // Dummy Conversion, read vms, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(9, ADC0, EMUX_AB7, 0,           0);     // Dummy Conversion, read vms, then change CAFE MUX to vms
...
#endif // #if (BOARD_SELECT == BOARD_PAC5556AEVK1_1SHUNT)
