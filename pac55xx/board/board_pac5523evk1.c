...
void init_diff_amp_gain(void)
{
    ...
    pac5xxx_tile_register_write(ADDR_CFGAIO1, (AIO50_LPROT_PR1PR2 | (AIO50_OFFSET_ENABLED << 3) | AIO50_HPCOMP_1US));
#elif (SEL_OC_METHOD == OC_METHOD_HPDAC)
    pac5xxx_tile_register_write(ADDR_CFGAIO1, (AIO50_HPROT_PR1 | (AIO50_OFFSET_ENABLED << 3) | AIO50_HPCOMP_2US));
#else
...
#ifdef ENABLE_CBC_CURRENT_LIMIT    
    pac5xxx_tile_register_write(ADDR_CFGAIO3, (AIO50_LPROT_PR1PR2 | (AIO50_OFFSET_ENABLED << 3) | AIO50_HPCOMP_1US));
#elif (SEL_OC_METHOD == OC_METHOD_HPDAC)
    pac5xxx_tile_register_write(ADDR_CFGAIO3, (AIO50_HPROT_PR1 | (AIO50_OFFSET_ENABLED << 3) | AIO50_HPCOMP_2US));
#else    
...
#ifdef ENABLE_CBC_CURRENT_LIMIT     
    pac5xxx_tile_register_write(ADDR_CFGAIO5, (AIO50_LPROT_PR1PR2 | (AIO50_OFFSET_ENABLED << 3) | AIO50_HPCOMP_1US));
#elif (SEL_OC_METHOD == OC_METHOD_HPDAC)
    pac5xxx_tile_register_write(ADDR_CFGAIO5, (AIO50_HPROT_PR1 | (AIO50_OFFSET_ENABLED << 3) | AIO50_HPCOMP_2US));
#else    
     pac5xxx_tile_register_write(ADDR_CFGAIO5, (AIO50_LPROT_PR1 | (AIO50_OFFSET_ENABLED << 3)));   // both high and low side off
#endif    
...
void config_seq_foc(void)
{
    ...
    FOC_PWM_Timer_Ptr->CCTR1.CTR = 5;                  // advance 0.1us from the middle
    #else
    FOC_PWM_Timer_Ptr->CCTR1.CTR = 25;                  // advance 0.5us from the middle
    #endif
    PAC55XX_ADC->DTSETRIGENT0TO3.FORCE1 = 0;            // Reset to 0
    PAC55XX_ADC->DTSETRIGENT0TO3.TRIG1CFGIDX = 10;      // DTSE Trigger 0 Sequence Configuration Entry Index
    ...
    #else
    pac5xxx_dtse_seq_config(10, ADC0, EMUX_AB7, 0,           0);       // Dummy Conversion, then change CAFE MUX to M1-Vu
    pac5xxx_dtse_seq_config(11, ADC0, EMUX_AB8, 0,           0);       // Convert M1-Vu, then change CAFE MUX to M1-Vv
    pac5xxx_dtse_seq_config(12, ADC0, EMUX_AB8, 0,           0);       // Dummy Conversion, then change CAFE MUX to M1-Vv
    pac5xxx_dtse_seq_config(13, ADC0, EMUX_AB9, 0,           0);       // Convert M1-Vv, then change CAFE MUX to M1-Vw
    pac5xxx_dtse_seq_config(14, ADC0, EMUX_AB9, 0,           0);       // Dummy Conversion, then change CAFE MUX to M1-Vw
    pac5xxx_dtse_seq_config(15, ADC0, EMUX_AB7, 0,     SEQ_END);       // Convert M1-Vw, then change CAFE MUX to M1-Vu, generate ADC interrupt
    #endif
#endif
}
...
void config_seq_foc_vref(void)
{
#define SELECT_V_SOURCE    0x06 //0x06//0x04 //0x01 //0x03 //0x01 //0x06    // Select VPSTAT
    unsigned char value;
...
#if 1
    // Dummy conversions are added to each sequence to work around a bug in the DTSE
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_VREFDIV2, 0,           0);    // Convert Vout, then change CAFE MUX to M1-Iu
    pac5xxx_dtse_seq_config(1, ADC0, EMUX_VREFDIV2, 0,           0);    // Dummy Conversion, then change CAFE MUX to M1-Iu
    pac5xxx_dtse_seq_config(2, ADC0, EMUX_VREFDIV2, 0,           0);    // Convert M1-Iu, then change CAFE MUX to M1-Iv
    pac5xxx_dtse_seq_config(3, ADC0, EMUX_VREFDIV2, 0,           0);    // Dummy Conversion, then change CAFE MUX to M1-Iv
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_VREFDIV2, 0,           0);    // Convert M1-Iv, then change CAFE MUX to M1-Iv
    pac5xxx_dtse_seq_config(5, ADC0, EMUX_VREFDIV2, 0,           0);    // Dummy Conversion, then change CAFE MUX to M1-Iw
    pac5xxx_dtse_seq_config(6, ADC0, EMUX_VREFDIV2, ADC_IRQ0_EN, 0);    // Convert M1-Iw, then change CAFE MUX to M1-Iu, generate ADC interrupt
    pac5xxx_dtse_seq_config(7, ADC6, EMUX_VREFDIV2,                 0,     SEQ_END);    // Dummy conversion, then change CAFE MUX to M1-Iu
#endif

// Configure Power Mux so AB11 = Vout(Vbus)
//    value = pac5xxx_tile_register_read(ADDR_PWRCTL);
//    value = (value & 0xC7) | (0x05 << 3);
    pac5xxx_tile_register_write(ADDR_PWRCTL, 0);     // Select PWRMON Mux = Vbus
    value = (SELECT_V_SOURCE << 3) & (0x38);
...
#endif // #if (BOARD_SELECT == BOARD_PAC5523EVK1)
