...
#if (BOARD_SELECT == BOARD_PAC5526AEVK1)
void init_diff_amp_gain(void)
{
    Struct_Motor* motor_ptr = &motor;
    
    pac5xxx_tile_register_write(ADDR_CFGAIO0, ((((AIO50_MODE_DIFFAMP << 6) | (AIO50_GAIN_X1 << 3)) & 0xC7) | (motor_ptr->app.iphase_amp_gain << 3)) & (~AIO50_LPCOMP_DISABLE));
    // Disable HPROT
    pac5xxx_tile_register_write(ADDR_CFGAIO1, (AIO50_LPROT_PR | (AIO50_OFFSET_ENABLED << 3)) & (~0x03));

    pac5xxx_tile_register_write(ADDR_CFGAIO2, ((((AIO50_MODE_DIFFAMP << 6) | (AIO50_GAIN_X1 << 3)) & 0xC7) | (motor_ptr->app.iphase_amp_gain << 3)) & (~AIO50_LPCOMP_DISABLE));
    // Disable HPROT
    pac5xxx_tile_register_write(ADDR_CFGAIO3, (AIO50_LPROT_PR | (AIO50_OFFSET_ENABLED << 3)) & (~0x03));
    ...
    // Disable HPROT
    pac5xxx_tile_register_write(ADDR_CFGAIO5, (AIO50_LPROT_PR | (AIO50_OFFSET_ENABLED << 3)) & (~0x03));
...
void config_seq_foc(void)
{
    ...
    motor.fptr.pSeq_add = (ADC_RESULT_TYPEDEF*) &PAC55XX_ADC->DTSERES9;
#else
    ...
    PAC55XX_ADC->DTSETRIGENT0TO3.TRIG0EDGE = 2;         // Trigger rising edge
#endif

    // Dummy conversions are added to each sequence to work around a bug in the DTSE
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0, 0);    // Convert M1_Iu, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(1, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0, 0);    // Dummy Conversion, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(2, ADC0, EMUX_AIO54, 0, SEQ_END);              // Dummy Conversion, then change CAFE MUX to M1-I
    ...
    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0,           0);    // Convert M1-I, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, ADC_IRQ0_EN, 0);    // Dummy Conversion, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(5, ADC6, EMUX_AIO54, 0, 0);    // Convert Iw, then change CAFE MUX vms, generate ADC interrupt
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AB13, 0, 0);    // Dummy Conversion, change CAFE MUX to vms
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB13, 0,  0);    // Dummy Conversion, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB13, 0,  0);    // Convert vms
    pac5xxx_dtse_seq_config(9, ADC0, EMUX_AIO54, 0, SEQ_END);                         // Convert vms, then change CAFE MUX to M1-Iu
    ...
    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0,           0);    // Convert M1-I, then change CAFE MUX to M1-I
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, ADC_IRQ0_EN, 0);    // Dummy Conversion, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(5, ADC6, EMUX_AIO54, 0, 0);    // Convert Iw, then change CAFE MUX vms, generate ADC interrupt
    pac5xxx_dtse_seq_config(6, ADC6, EMUX_AB13, 0, 0);    // Dummy Conversion, change CAFE MUX to vms
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB13, 0,  0);    // Dummy Conversion, then change CAFE MUX to vms
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB13, 0,  0);    // Convert vms
...
void config_seq_ipd(uint32_t sequencer_trigger_value)
{
    // Set Vbus sequencer slot
#if (INTERNAL_VMS == TRUE)
    motor.fptr.pSeq_add = (ADC_RESULT_TYPEDEF*) &PAC55XX_ADC->DTSERES7;
#else
    #error
#endif
    ...
    // Dummy conversions are added to each sequence to work around a bug in the DTSE
    pac5xxx_dtse_seq_config(0, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, 0,           0);    // Dummy Convert Vout, then change CAFE MUX to M1-Iu
    pac5xxx_dtse_seq_config(1, ADC0, EMUX_AIO54 | DIFFAMPS_HOLD, ADC_IRQ0_EN, 0);    // Dummy Conversion, then change CAFE MUX to M1-Iu
    pac5xxx_dtse_seq_config(2, ADC0, EMUX_AIO54, 0,           0);    // Convert M1-Iu, then change CAFE MUX to M1-Iv
    pac5xxx_dtse_seq_config(3, ADC0, EMUX_AIO54, 0,           0);    // Dummy Conversion, then change CAFE MUX to M1-Iv
    pac5xxx_dtse_seq_config(4, ADC0, EMUX_AB13, 0, 0);    // Convert M1-Iv, then change CAFE MUX to M1-Iv
    pac5xxx_dtse_seq_config(5, ADC0, EMUX_AB13, 0, 0);    // Convert Iu, then change CAFE MUX vms, 
    pac5xxx_dtse_seq_config(6, ADC0, EMUX_AB13, 0, 0);    // Dummy
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AIO54, 0, SEQ_END);        // Convert vms, then change CAFE MUX to M1-Iu
}
...
void config_seq_sim(void)
{
    ...
    pac5xxx_dtse_seq_config(6, ADC0, EMUX_AB13, ADC_IRQ0_EN, 0);       // Convert M1-Vw, then change CAFE MUX to M1-Vu, generate ADC interrupt
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB13, 0, 0);    // Dummy
    pac5xxx_dtse_seq_config(8, ADC6, EMUX_AB7, 0,     SEQ_END);       // Dummy conversion, then change CAFE MUX to M1-Vu
}
...
#endif // #if (BOARD_SELECT == BOARD_PAC5526AEVK1)
