...
#if (BOARD_SELECT == BOARD_PAC5527HCRD1_ENG)
void init_diff_amp_gain(void)
{
    Struct_Motor* motor_ptr = &motor;
    
    pac5xxx_tile_register_write(ADDR_CFGAIO0, ((((AIO50_MODE_DIFFAMP << 6) | (AIO50_GAIN_X1 << 3) | AIO50_LPCOMP_4US) & 0xC7) | (motor_ptr->app.iphase_amp_gain << 3)));
    ...
    pac5xxx_tile_register_write(ADDR_CFGAIO2, ((((AIO50_MODE_DIFFAMP << 6) | (AIO50_GAIN_X1 << 3) | AIO50_LPCOMP_4US) & 0xC7) | (motor_ptr->app.iphase_amp_gain << 3)));
#ifdef ENABLE_CBC_CURRENT_LIMIT    
    ...
    pac5xxx_tile_register_write(ADDR_CFGAIO4, ((((AIO50_MODE_DIFFAMP << 6) | (AIO50_GAIN_X1 << 3) | AIO50_LPCOMP_4US) & 0xC7) | (motor_ptr->app.iphase_amp_gain << 3)));
#ifdef ENABLE_CBC_CURRENT_LIMIT     
...
void config_seq_sim(void)
{
    ...
    pac5xxx_dtse_seq_config(6, ADC0, EMUX_AB13, ADC_IRQ0_EN, 0);     // Convert M1-Iw, then change CAFE MUX to M1-Iu, generate ADC interrupt
    pac5xxx_dtse_seq_config(7, ADC0, EMUX_AB13, 0, 0);               // Dummy Conversion, change CAFE MUX to vms
    pac5xxx_dtse_seq_config(8, ADC0, EMUX_AB7, 0, SEQ_END);        // Convert vms, then change CAFE MUX to M1-Vu
}
...
#endif // #if (BOARD_SELECT == BOARD_PAC5527HCRD1_ENG)
