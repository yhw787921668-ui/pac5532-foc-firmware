...
#if (DEVICE_SELECT == DEVICE_5256)
//====================================
// CONFIG: VP voltage setting
//====================================
// Configure VP as default as User Guide
#define CONFIG_VP_VOLTAGE               VP_VOLTAGE_15V
...
void cafe_init(void)
{
   // Configure SOC Bridge for talking to CAFE
    ...
    // Disable HS drivers on H-PROT (Bit 3), Disable LS drivers on L-PROT (Bit 2)
...
{
    // This function is empty for 5532 
}
...
#if (BOARD_SELECT == BOARD_PAC5256EVK1_REV_AB) || (BOARD_SELECT == BOARD_PAC5256EVK1_REV_AB_1SHUNT)
    hpdac_value = (fix16_mul((fix16_t) ((lpdac_value - 512) << 16), (fix16_t) (QCONST(HPDAC_OC_LIMIT_MULTIPLIER, 16))) >> 16);
    hpdac_value = hpdac_value + 512;
#endif
...
#endif // #ifdef DEVICE_SELECT == DEVICE_5256
