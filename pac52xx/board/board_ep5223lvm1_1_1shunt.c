...
#if (BOARD_SELECT == BOARD_EP5223LVM1_1_1SHUNT)
...
void config_seq_foc(void)
{
    ...
    pac5xxx_adc_as1_sequence_config(1, ADCCTL_ADMUX_AD4, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32), ASSEQ_DELAY_0);                            // Convert Vbus, then change CAFE MUX to M1-Iu
...
void config_seq_ipd(uint32_t sequencer_trigger_value)
{
    ...
    pac5xxx_adc_as0_sequence_config(0, ADCCTL_ADMUX_AD0, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32) | SIGMGR_MSPI_SH_DIFFAMP, ASSEQ_DELAY_0);     // Convert M1-Iu, then change CAFE MUX to M1-Vu
    ...
    pac5xxx_adc_as1_sequence_config(0, ADCCTL_ADMUX_AD4, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32), ASSEQ_DELAY_0);                            // Convert Vbus, then change CAFE MUX to M1-Iu
    pac5xxx_adc_as1_sequence_config(1, ADCCTL_ADMUX_AD0, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32), ASSEQ_DELAY_0);                            // change CAFE MUX to M1-Iu
#elif (POWER_MEASURE_SELECT == POWER_MEASURE_METHOD_2)
#error: POWER_MEASURE_METHOD_2 not supported BOARD_EP5223LVM1_1_1SHUNT
#endif
...
#endif // #if (BOARD_SELECT == BOARD_EP5223LVM1_1)
