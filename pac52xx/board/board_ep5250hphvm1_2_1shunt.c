...
#if (BOARD_SELECT == BOARD_EP5250HPHVM1_2_1SHUNT)
...
void config_seq_foc(void)
{
    ...
    pac5xxx_adc_as1_sequence_config(1, ADCCTL_ADMUX_AD2, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32), ASSEQ_DELAY_0);   // Convert Vbus, then change CAFE MUX to M1-Iu
...
void config_seq_ipd(uint32_t sequencer_trigger_value)
{
    ...
    // AS1 configuration
    pac5xxx_adc_as1_config_pwm(ADCCTLX_AS1D_DEPTH2, ADCCTLX_TRIGEDGE_HITOLOW, ADCCTLX_TRIGPWM_PWMA1);
    pac5xxx_adc_as1_sequence_config(0, ADCCTL_ADMUX_AD2, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32), ASSEQ_DELAY_0);   // Convert Vbus, then change CAFE MUX to M1-Iu
    pac5xxx_adc_as1_sequence_config(1, ADCCTL_ADMUX_AD0, ASSEQ_MSPI_AFTER_SH, SIGMGR_MSPI(SIGMGR_AIO32), ASSEQ_DELAY_0);   // Convert I2, then change CAFE MUX to Vbus
}
