...
/**
 * @file startup_aligngo2trap_funcs.h
 * @brief Use for Align startup functions
 */
...
#ifndef STARTUP_ALIGNGO2TRAP_FUNCS_H
#define STARTUP_ALIGNGO2TRAP_FUNCS_H
...
{
    int32_t temp_angle_q16;
    // Set aligngo fixed command values
#ifdef ENABLE_IPD
    // To do: Trap start with IPD
    motor_ptr->startup.aligngo.alignangle_cmd_q26 = motor_ptr->ipd.ipd_starting_angle;
#else
    // This needs to match with the modulator start sector
    motor_ptr->startup.aligngo.alignangle_cmd_q26 = (FIX16_PI_DIV_2 << 10);
#endif
    // Convert q26 to q16
    temp_angle_q16 = fix16_mul(motor_ptr->startup.aligngo.alignangle_cmd_q26, QCONST(0.000976, Q16));
    wraparound(&temp_angle_q16);
    // Determine the start bemf state with the initial angle
    trap_calc_sector_from_angle(temp_angle_q16, motor_ptr);
    ...
    if (motor_ptr->sim.status_sim & (STATUS_SIM_TIMEOUT | STATUS_SIM_BRAKECOMPLETE | STATUS_SIM_WAITSTOPCOMPLETE))
    {
        ...
        // Set extra current
        motor_ptr->mpos.aligngo.idrefol_target_q16 = fix16_mul(motor_ptr->mpos.aligngo.idrefol_target_q16, motor_ptr->sim.OL_Id_ref_braking_mul);
        temp_value = fix16_mul(motor_ptr->mpos.aligngo.idrefol_target_q16, motor_ptr->sim.OL_Id_ref_braking_mul);
    ...
    switch(motor_ptr->startup.state)
    {
        ...
        case STATE_STARTUP_ALIGNGO_RUN:
            ...
            if ((motor_ptr->mpos.aligngo.status == STATUS_ALIGNGO_COMPLETE) && motor_ptr->app.auto_close_loop_cmd)
            {
                ...
                // Reset startup count for trap stall detection
                motor_ptr->mpos.trap.startup_stall_cnt = 0;
            }
            break;

        case STATE_STARTUP_ALIGNGO_ANGLEDIFF:
            // debugio1_toggle();
            // Bypass anglediff_check() in trap transition, which is handled in the faster timelevel for BEMF detection
//            anglediff_check(motor_ptr, (motor_ptr->mpos.aligngo.alignangle_q26 >> 10), motor_ptr->mpos.est.estimated_angle, motor_ptr->mpos.aligngo.speedrefol_target_radps_q16, motor_ptr->mpos.est.est_spd_radps);
            // AngleDiff Check for Aligngo to Trap Switchover
            if (motor_ptr->mpos.anglediff_pass_flag)
            {
                ...
                motor_ptr->status.motor |= (STATUS_MOTOR_CLOSED_LOOP | STATUS_MOTOR_MPOSMODE_TRAP);
...
#endif  //#if (MPOS_SELECT == MPOS_LBG1_START_TRAP_ALIGNGO)
#endif  //#ifndef STARTUP_ALIGNGO2TRAP_FUNCS_H
