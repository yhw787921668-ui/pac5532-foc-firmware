...
#ifndef SIM_V_FUNCS_H
#define SIM_V_FUNCS_H
...
*/
extern void sim_v_detect_brakestop(Struct_SIM* sim_ptr);
...
* @param Struct_SIM *sim_ptr
*/
extern void sim_v_detect_waitstop(Struct_SIM* sim_ptr);
...
* @param Struct_SIM *sim_ptr
*/
extern void sim_v_mdetect(Struct_SIM *sim_ptr);
...
* @param Struct_SIM *sim_ptr
*/
extern void sim_v_detect2(Struct_SIM *sim_ptr);


//=====================================
// Function Declarations (sim_v_funcs.c)
//=====================================
...
/**
* @brief Calculates amplifier gain based on user configuration
* @param unsigned char amp_gain
* @return rv
*/
extern unsigned char sim_conv_ampgainval2gain(unsigned char amp_gain);
...
/**
* @brief Returns the U-phase voltage used for SIM detection
* @param Struct_Motor* motor_ptr
* @return readseq_vphase_u
*/
static inline int16_t readseq_sim_vphase_u(void)               { return readseq_vphase_u(); }


/**
* @brief Returns the V-phase voltage used for SIM detection
* @param Struct_Motor* motor_ptr
* @return readseq_vphase_v
*/
static inline int16_t readseq_sim_vphase_v(void)               { return readseq_vphase_v(); }


/**
* @brief Returns the W-phase voltage used for SIM detection
* @param Struct_Motor* motor_ptr
* @return readseq_vphase_w
*/
static inline int16_t readseq_sim_vphase_w(void)               { return readseq_vphase_w(); }
...
// JAGTAG: sim_brake_reset() may be the same for both i and v
/**
...
// JAGTAG: sim_waitstop_reset() may be the same for both i and v
/**
...
static inline void sim_read_inputs(Struct_Motor *motor_ptr)
{
    ...
    // Convert to scaled voltage referenced to 0 (Range = [0, +1])
    motor_ptr->sim.vphase_u_q14 = (readseq_sim_vphase_u() * motor_ptr->sim.multp_conv_adc2vscaled_q16) >> 2;    // q0 * q16 = q16 >>2 = q14
    motor_ptr->sim.vphase_v_q14 = (readseq_sim_vphase_v() * motor_ptr->sim.multp_conv_adc2vscaled_q16) >> 2;    // q0 * q16 = q16 >>2 = q14
    motor_ptr->sim.vphase_w_q14 = (readseq_sim_vphase_w() * motor_ptr->sim.multp_conv_adc2vscaled_q16) >> 2;    // q0 * q16 = q16 >>2 = q14

    // Calculate centertap
    motor_ptr->sim.centertap_q14 = (((motor_ptr->sim.vphase_u_q14 + motor_ptr->sim.vphase_v_q14 + motor_ptr->sim.vphase_w_q14) * ONE_THIRD_IN_FIX16) >> 16);

    // Reference phase voltage to centertap (Range  = +/-1)
    motor_ptr->sim.vphase_u_q14 = ((motor_ptr->sim.vphase_u_q14 - motor_ptr->sim.centertap_q14) << 1);
    motor_ptr->sim.vphase_v_q14 = ((motor_ptr->sim.vphase_v_q14 - motor_ptr->sim.centertap_q14) << 1);
    motor_ptr->sim.vphase_w_q14 = ((motor_ptr->sim.vphase_w_q14 - motor_ptr->sim.centertap_q14) << 1);
}


static inline void sim_mdetect(Struct_Motor *motor_ptr)
{
    sim_v_mdetect(&motor_ptr->sim);
}
...
static inline void sim_detect2(Struct_Motor *motor_ptr)
{
    ...
    sim_v_detect2(&motor_ptr->sim);

#if defined(CONFIG_SIM_VOLTAGE_KE)
...
#else
    // Calculate BEMF-based voltage vector from SIM alpha/beta
    park_transform_q14(motor_ptr->sim.sim_angle, (motor_ptr->sim.sim_alpha_q14 * motor_ptr->sim.sim_motor_direction), (motor_ptr->sim.sim_beta_q14 * motor_ptr->sim.sim_motor_direction), &temp, &motor_ptr->sim.ve_mag_scaled_q14);
#endif
...
static inline void sim_sim2cl(Struct_Motor *motor_ptr)
{
    ...
    wraparound(&motor_ptr->sim.sim_angle);
//    motor_ptr->mpos.mpos_angle = motor_ptr->sim.sim_angle;    // For debuggin purposes
...
{
    sim_v_detect_brakestop(&motor_ptr->sim);
...
{
    sim_v_detect_waitstop(&motor_ptr->sim);
}
