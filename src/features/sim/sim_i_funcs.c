...
/**
* @file sim_i_funcs.c
* @brief I-SIM function definitions
*/
...
void sim_calculate_parameters(Struct_Motor* motor_ptr)
{
    ...
    // Calculate motion detect duty decrement per SIM dt
    temp1 = fix16_div(SIM_MDETECT_DUTYRAMPTIME_SEC_Q8 << 8, (motor_ptr->sim.dt_control_q26)) >> 6;   // temp1 = Ramp counts in SIM dt's
    motor_ptr->sim.mdetect_duty_cycle_dec_q14 = (fix16_div((QCONST(1.0, Q14) - motor_ptr->sim.braking_duty_min_q14) << 8, temp1) >> 24);   // decrement = swing / counts
    if(motor_ptr->sim.mdetect_duty_cycle_dec_q14 <= 0)
        motor_ptr->sim.mdetect_duty_cycle_dec_q14 = 1;
    else if(motor_ptr->sim.mdetect_duty_cycle_dec_q14 > QCONST(1.0, Q14))
        motor_ptr->sim.mdetect_duty_cycle_dec_q14 = QCONST(1.0, Q14);
}
...
void sim_init(Struct_Motor* motor_ptr)
{
    ...
    motor_ptr->sim.phase_adc_threshold_q14 = ((SIM_BEMF_DET_THRESHOLD_ADCCNT_I * CONV_Q10TOQ14) >> 14);
}
